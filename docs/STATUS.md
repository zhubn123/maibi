...
项目处于首版基础建设阶段。已完成产品计划、协作规范、Python 项目骨架、`core` 共享接口、`server` 最小 FastAPI 服务、Mock ASR 集成测试、客户端 UI 状态模型、PCM 分片骨架和麦克风采集适配层；正在推进腾讯云 ASR 签名和可体验客户端壳。
...
- PR #9：Tencent ASR signer
  - 状态：Draft
  - 内容：新增腾讯云实时 ASR WebSocket 签名 URL 构建和脱敏测试
- PR #10：Demo client shell
  - 状态：开发中
  - 内容：新增可启动的 PySide6 托盘/浮窗模拟体验入口

## 下一步

1. 完成并合并 PR #9。
2. 完成并合并 PR #10。
3. 后续接入真实 ASR WebSocket session 和文本上屏能力。
